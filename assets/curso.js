...
(function(){
  ...
  // PATCH CCPN (curso de 10 módulos, > teto de 9 do motor herdado): sort() default é string
  // ("1","10","2"...) — troca pra sort numérico. Único desvio da regra "não tocar no roteador",
  // necessário porque o motor original assume no máx. 9 aulas. Ver CONTRATO-FRAGMENTO.md.
  var KEYS=Object.keys(aulaEls).sort(function(a,b){ return parseInt(a,10)-parseInt(b,10); });
  ...
  var active=null;
  function show(route){
    // PATCH CCPN: regex original só aceitava 1 dígito (aula-1..aula-9); curso tem 10 módulos.
    if(!/^(trilha|aula-[1-9][0-9]?)$/.test(route)) route='trilha';
